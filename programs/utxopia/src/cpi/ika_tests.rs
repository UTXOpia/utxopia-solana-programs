...
fn approve_message_ix_data_layout_taproot() {
    ...
    let data =
        build_approve_message_ix_data(bump, &sighash, &metadata, &user, SIG_SCHEME_TAPROOT_SHA256);
...
fn approve_message_ix_data_zero_inputs_round_trip() {
    let zeros = [0u8; 32];
    let data = build_approve_message_ix_data(0, &zeros, &zeros, &zeros, SIG_SCHEME_ECDSA_SHA256);
    assert_eq!(data[0], IX_APPROVE_MESSAGE);
    assert_eq!(data[1], 0);
...
fn approve_message_ix_data_distinguishes_metadata_from_user() {
    ...
    let data = build_approve_message_ix_data(7, &sighash, &meta, &user, SIG_SCHEME_TAPROOT_SHA256);
